...
5. **Aucun secret dans le dépôt.** Les clés Supabase transitent par `.env` (gitignoré) → `runtimeConfig.public`. Le keystore de signature et ses mots de passe passent par `android/keystore.properties` (gitignoré). Les copies maîtresses des deux vivent dans `.arpente-secrets/`, à la racine du conteneur `Projets/`.
